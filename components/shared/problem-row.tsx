...
export function ProblemRow({
    ...
    const handleStatusClick = (e: React.MouseEvent) => {
        ...
        console.log("🖱️ ProblemRow: Status clicked:", { id, name, currentStatus: status });

        // If transitioning from todo to solved, show difficulty modal
        if (status === "todo" && onMarkSolved) {
            console.log("🖱️ ProblemRow: Opening difficulty modal...");
            setIsModalOpen(true);
        } else {
            // Otherwise, cycle normally
            console.log("🖱️ ProblemRow: Cycling status...");
            onCycleStatus(id);
        }
    };

    const handleDifficultySelect = (rating: DifficultyRating) => {
        console.log("🎨 ProblemRow: Difficulty selected:", { id, name, rating });
        if (onMarkSolved) {
            console.log("🎨 ProblemRow: Calling onMarkSolved...");
            onMarkSolved(id, rating);
        } else {
            console.warn("⚠️ ProblemRow: onMarkSolved is not defined!");
        }
    };
